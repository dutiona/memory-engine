...
low-entropy secrets that carry no signal, with near-zero false positives. The denylist
values are supplied **at runtime from a gitignored source and are never committed** to this
crate; only fake fixtures live in `#[cfg(test)]`. A `DENYLIST_MIN_LEN = 4` guard prevents a
stray short/blank line in the source from nuking the corpus.
