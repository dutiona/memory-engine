...
**Important caveat (C2 from critic):** the `SpaceEpochCounter` guards embedding-space swaps but not edge-weight changes. When sim-graph k or W is retuned without a full promote, the Laplacian and ORC eigenbasis go silently stale. The `DerivedStructureRegistry` row for every spectral/curvature structure must carry a **content fingerprint** = hash{epoch, W-version, k, scope-row-count-bucket}, and invalidate on fingerprint mismatch, not epoch alone. This is the single most likely source of a "looks fresh, is corrupt" spectral result.
...
Content fingerprint: every structure in the registry carries `hash{epoch, W-version, k, scope-row-count-bucket}`; invalidation checks the fingerprint, not epoch alone.
...
| **E2** | Spectral and curvature layer | Batch offline structures for recall and bridge detection | Laplacian eigenbasis + diffusion-map coordinates + diffusion kernel cache; ORC per-edge cache; spectral cluster labels + centroids; `MaterializationScheduler` with content-fingerprint invalidation | GFT (Shuman et al. 2013; Sandryhaila and Moura 2014); spectral clustering (Ng et al. 2001; von Luxburg 2007); Chebyshev filtering (Defferrard et al. 2016; Hammond et al. 2011); ORC (Ollivier 2009; Ni et al. 2019; Topping et al. 2022); diffusion distance (Coifman et al. 2005/2006) | Diffusion-distance vs cosine recall PoC (sweep t); ORC bridge map PoC (list 20 most-negative edges, manually verify cross-domain). Decision: diffusion-distance beats cosine on multi-hop recall at some t ⇒ replace/augment ranker; most-negative edges are real bridges ⇒ ship ORC + curvature-protected forgetting. |
