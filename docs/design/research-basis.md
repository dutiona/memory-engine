...
| **Graphiti**      | 2410.13790 | Bi-temporal model with 4 timestamps per fact: `t_created`/`t_expired` (system time, when the engine learned/forgot it) and `t_valid`/`t_invalid` (real-world time, when the fact was/becomes true). Conflict detection delegated to LLM via trait.                                                                                                                            |
...
| **SparseMemFT**   | --         | Sparse memory fine-tuning achieves only 11% knowledge drop (vs 89% full fine-tuning, 71% LoRA). Informed the boundary decision: the engine is retrieval-only. Parameter updates (fine-tuning, LoRA adaptation) are explicitly out of scope.                                                                                                                                   |
