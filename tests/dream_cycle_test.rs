...
fn add(engine: &MemoryEngine, content: &str, ft: FactType, importance: f64) -> i64 {
    ...
    engine.add_fact(&req, &TagEmbed, None).unwrap()
}

#[test]
fn full_cycle_produces_and_applies_then_reruns_as_noop() {
    ...
    let neg_id = add(&engine, "neg fact", FactType::Episodic, 0.5);
    for _ in 0..3 {
        engine.record_outcome(neg_id, Outcome::Negative).unwrap();
    }
    // A useful fact → rescore up.
    let pos_id = add(&engine, "pos fact", FactType::Episodic, 0.5);
    engine.record_outcome(pos_id, Outcome::Positive).unwrap();
    engine.record_outcome(pos_id, Outcome::Positive).unwrap();
    ...
    assert_eq!(
        engine.explain_fact(neg_id).unwrap().state,
        FactState::Expired {
    ...
    let report2 = engine.run_dream_cycle(&cycle).unwrap();
    assert!(
...
#[test]
fn run_dream_cycle_on_read_only_engine_is_rejected() {
