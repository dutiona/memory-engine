...
//!
//! [`apply_cycle_report`] validates the entire delta log against a read-only
//! snapshot, then applies every delta in a **single transaction** so a
//! malformed delta leaves the store byte-identical.
//!
//! Lock discipline: one atomic port call for the whole operation. `Promote` reuses
//! the shared atomic-promote pipeline and `TagOutcome` inserts the outcome event
//! directly on the shared transaction — neither re-acquires a lock, which would
//! self-deadlock on a non-reentrant connection.

use parking_lot::RwLock;

use me_index::{EdgeData, MemoryGraph};
use me_storage::{MemoryCtx, UpcasterRegistry};
use me_types::error::Result;
use me_types::types::RelationType;
use me_types::types::cycle_report::{ApplyResult, CycleReport};
...
/// ([`ConsolidationStore::apply_cycle_deltas_atomic`](me_storage::ConsolidationStore::apply_cycle_deltas_atomic)),
/// which also fires the post-commit HNSW notify (Stage B). If any delta fails
/// validation the store is left **unchanged**. The caller consumes only the returned
...
/// # Errors
///
/// - [`MemoryError::ReadOnly`](me_types::error::MemoryError::ReadOnly) if the engine is read-only.
/// - [`MemoryError::Cycle`](me_types::error::MemoryError::Cycle) if any delta fails validation.
/// - [`MemoryError::EmbeddingDimension`](me_types::error::MemoryError::EmbeddingDimension) if an
...
mod tests {
    ...
    use me_types::types::cycle_report::{CycleDelta, CycleMetadata, IdentityOutput, TimeWindow};
    use me_types::types::{FactType, NewFact, Outcome, PromotionProvenance};
    ...
    async fn importance_of(engine: &TestEngine, id: i64) -> f64 {
    ...
    async fn apply(engine: &TestEngine, report: &CycleReport) -> Result<ApplyResult> {
        apply_cycle_report(
            engine.ctx(),
            &engine.graph,
    ...
    async fn apply_add_fact_into_unstamped_store_is_rejected() {
        ...
        // rejected. Uses a raw (unstamped) TestEngine, bypassing the identity-seeding
        // `engine()` helper.
        let engine = TestEngine::new(DIM).await;
        let nf = NewFact {
        ...
        let err = apply(&engine, &report(vec![CycleDelta::AddFact(nf)], vec![]))
            .await
            .unwrap_err();
        assert!(
    ...
    async fn empty_report_is_ok_noop() {
        let engine = engine().await;
        let res = apply(&engine, &report(vec![], vec![])).await.unwrap();
    ...
    async fn adjust_score_moves_base_importance_and_clamps() {
        let engine = engine().await;
        let id = engine.add("f").await; // default importance 0.5
        apply(
            &engine,
            &report(
    ...
    async fn quarantine_expires_and_marks_but_row_survives() {
        let engine = engine().await;
        let id = engine.add("bad fact").await;
        apply(
            &engine,
            &report(
    ...
    async fn supersede_expires_old_and_creates_edge() {
        let engine = engine().await;
        let old = engine.add("old fact").await;
        let new = engine.add("new fact").await;
        apply(
            &engine,
            &report(
    ...
    async fn synthesize_inserts_summary_expires_sources_and_creates_edges_and_lineage() {
        let engine = engine().await;
        let s1 = engine.add("source one").await;
        let s2 = engine.add("source two").await;
        let res = apply(
            &engine,
            &report(
    ...
    async fn synthesize_outputs_dream_marked_and_excluded_next_cycle_invariant_m() {
        let engine = engine().await;
        let s1 = engine.add("src a").await;
        let s2 = engine.add("src b").await;
        let res = apply(
            &engine,
            &report(
                vec![CycleDelta::Synthesize {
    ...
    async fn synthesize_requires_at_least_one_source() {
        ...
        assert!(
            engine.storage.list_all_facts().await.unwrap().is_empty(),
    ...
    async fn synthesize_missing_source_rejected() {
        let engine = engine().await;
        let s1 = engine.add("real source").await;
        let err = apply(
            &engine,
            &report(
                vec![CycleDelta::Synthesize {
    ...
    async fn synthesize_expired_source_rejected() {
        let engine = engine().await;
        let s1 = engine.add("expire me first").await;
        apply(
            &engine,
            &report(
                vec![CycleDelta::Quarantine {
    ...
    async fn synthesize_wrong_dimension_rejected() {
        let engine = engine().await;
        let s1 = engine.add("source").await;
    ...
    async fn synthesize_lineage_provenance_spans_sources_not_synthetic_instant() {
        let engine = engine().await;
        let s1 = engine.add("src one").await; // t_created ~ now
        let s2 = engine.add("src two").await;
        let mut nf = synthetic("merged");
    ...
    async fn synthesize_duplicate_source_across_deltas_is_rejected() {
        let engine = engine().await;
        let s1 = engine.add("a").await;
        let s2 = engine.add("b").await;
        let s3 = engine.add("c").await;
        let err = apply(
            &engine,
            &report(
                vec![
    ...
    async fn promote_and_tagoutcome_do_not_deadlock_and_apply() {
        // Regression guard for the non-reentrant-mutex traps: Promote reuses the
        // atomic promote pipeline and TagOutcome inserts on the shared tx — neither
        // re-acquires the write lock.
        let engine = engine().await;
        let id = engine.add("promote me").await;
        let res = apply(
            &engine,
            &report(
                vec![
        ...
        let counts = engine.storage.count_outcome_signals(id).await.unwrap();
    ...
    async fn add_fact_inserts_and_marks_processed() {
        ...
        let res = apply(&engine, &report(vec![CycleDelta::AddFact(nf)], vec![]))
            .await
            .unwrap();
    ...
    async fn apply_dream_marks_all_cycle_outputs_invariant_m() {
        let engine = engine().await;
        let a = engine.add("source to promote").await;
        let old = engine.add("to be superseded").await;
        let new = engine.add("supersede survivor").await;
    ...
    async fn add_fact_oversized_content_rejected() {
        let engine = engine().await;
        let huge = "x".repeat(me_types::limits::MAX_PAYLOAD_BYTES + 1);
        ...
        assert!(matches!(
            err,
            MemoryError::Conflict(me_types::error::ConflictError::PayloadTooLarge { .. })
        ));
    }
    ...
    async fn adjust_after_quarantine_in_same_report_is_rejected() {
        ...
        // in validation — otherwise the base-importance update (no t_expired guard)
        // would silently mutate an expired row.
        let engine = engine().await;
        let id = engine.add("f").await;
        let err = apply(
            &engine,
