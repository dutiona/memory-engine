...
async fn main() -> ExitCode {
    ...
    let result: anyhow::Result<ExitCode> = match cli.command {
        ...
        Commands::AddFact(ref args) => commands::add_fact::run(&cli.db, args, cli.format)
            .await
            .map(|()| ExitCode::SUCCESS),
        Commands::Consolidate(ref args) => commands::consolidate::run(&cli.db, args, cli.format)
            .await
            .map(|()| ExitCode::SUCCESS),
        Commands::BatchIngest(ref args) => commands::batch_ingest::run(&cli.db, args, cli.format)
            .await
            .map(|()| ExitCode::SUCCESS),
        Commands::Bootstrap(ref args) => commands::bootstrap::run(&cli.db, args, cli.format)
            .await
            .map(|()| ExitCode::SUCCESS),
